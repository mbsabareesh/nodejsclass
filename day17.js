...
con.query("create database if not exists day17_product",function(err){
         if (err) {
...
con.query("USE day17_product", function (err, result) {
        if (err){
            console.log(err.message);
        }else{
...
var item_array = [
    ...
    { "itemid":"","product_name": "smart watch", "product_id": "md-23", "amount_with_tax": 47200, "amount_without_tax": 40000, "gst": 18, "gst_amount": 7200, "CGST": 9, "SGST": 9, "IGST": null},
...
con.query(sql,function(err,result){
    try {
        ...
        {
                     var select1_sql="select * from order_table order by orderid desc limit 1";
                    ...
                    con.query(select_sql,function(err,result)
                    {
                        try 
                        {
                           if (err) 
                           {
                            console.log("error in insert delete update :"+err);
                           } else 
                           {
                              con.query(select1_sql,function(err,result){
                               try {
                                if (err) {
                                    console.log(err);
                                } else {
                            ...
                            var delete_array = result.filter(entry1 => !item_array.some(entry2=>(entry1.itemid == entry2.itemid)));
                            ...
                            if(insert_array.length>0)
                            {
                               insert_array.forEach(item =>
                                ...
                                var insert_sql = "INSERT INTO item_table (orderid, product_name, product_id, amount_with_tax, amount_without_tax, gst, gst_amount, CGST, SGST, IGST) VALUES ?";
                                            ...
                                            {
                                                if (err) console.log("error insert item"+err);
                            ...
                            if(update_array.length>0)
                            {
                                update_array.forEach(item =>
                                {
                                    ...
                                    set product_name = 'telephone', product_id = 'md-08', amount_with_tax = 10000, amount_without_tax = 10000,gst = 18, gst_amount = 1000, CGST = NULL, SGST = NULL, IGST = 18 WHERE cusid = 9`;
                                        ...
                                        con.query(update_sql, [orderid,item.product_name, item.product_id, item.amount_with_tax, item.amount_without_tax, item.gst, item.gst_amount, item.CGST, item.SGST, item.IGST], function(err, result) 
                                        {
